...
Each green point is real ground the system measured as lawn. There are no ground returns on a roof or a driveway, so the house and pavement exclude themselves. The outline is the lot extended to the curb.
...
Three data layers work together, so canopy, shadow, and hardscape don't throw the number off:
...
Not a notebook. It's a deployed, tested, continuously integrated service:

- **Ports-and-adapters architecture**, so new regions and data sources plug in without touching the core.
